...
describe('Get All order before date ', () => {
  ...
  it('should return all order before date in DB', async () => {
    // envoyer une requête HTTP GET sur l'url /api/mentoring-slots/was-missed
    // récupèrer la réponse HTTP

    const getOrdersBeforeDateResponse = await request(app.getHttpServer()).get('/api/orders/before');

    console.log(getOrdersBeforeDateResponse.error)

    // vérifier que la réponse a bien un status 200
    expect(getOrdersBeforeDateResponse.status).toBe(200);
